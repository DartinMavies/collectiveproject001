...
FRAGMENTS_ASSET = [FRAGMENT_MODEL, FRAGMENT_SURFACE, FRAGMENT_BINDING]
...
def main(args):
    ...
    for asset in ASSETS:
        ...
        asset_root_prim = pipe_globals.create_root_prim(
            asset_stage, "component", asset_name, asset_filename, "latest"
        )
        ...
        for purpose in PURPOSES:
            ...
            for fragment in FRAGMENTS_ASSET:
                ...
                pipe_globals.keep_folder(
                    pipe_globals.get_subfolder(fragment_folder, HOST_USD_EXPORT)
                )
                ...
                purpose_stage.GetRootLayer().subLayerPaths.insert(
                    0,
                    pipe_globals.anchor_path(
                        fragment_filename, purpose_stage.GetRootLayer().identifier
                    ),
                )
            ...
            asset_purpose_prim = UsdGeom.Scope.Define(
                asset_stage, asset_root_prim.GetPath().AppendChild(purpose)
            )
            asset_purpose_prim.GetPrim().GetAttribute("purpose").Set(purpose)
            asset_purpose_prim.GetPrim().GetReferences().AddReference(
                pipe_globals.anchor_path(purpose_filename, asset_stage.GetRootLayer().identifier)
            )
    ...
    for shot in SHOTS:
        ...
        shot_stage = pipe_globals.stage_begin(
            shot_filename,
            start=shot[START_TIMECODE],
            end=shot[END_TIMECODE],
            fps=shot[FRAMES_PER_SECOND],
        )
        ...
        for element in elements:
            ...
            if asset_name:
                ...
                asset_filename = os.path.join(asset_folder, "index.usda")
                # reference maybe?
                element_stage.GetRootLayer().subLayerPaths.insert(
                    0,
                    pipe_globals.anchor_path(
                        asset_filename, element_stage.GetRootLayer().identifier
                    ),
                )
            ...
            for fragment in FRAGMENTS_ELEMENT:
                ...
                element_stage.GetRootLayer().subLayerPaths.insert(
                    0,
                    pipe_globals.anchor_path(
                        fragment_filename, element_stage.GetRootLayer().identifier
                    ),
                )
            ...
            if composition_arc == COMPOSITION_ARC_REFERENCE:
                ...
                element_prim.GetPrim().GetReferences().AddReference(
                    pipe_globals.anchor_path(element_filename, shot_stage.GetRootLayer().identifier)
                )

            elif composition_arc == COMPOSITION_ARC_SUBLAYER:
                shot_stage.GetRootLayer().subLayerPaths.insert(
                    0,
                    pipe_globals.anchor_path(
                        element_filename, shot_stage.GetRootLayer().identifier
                    ),
                )
        ...
        for fragment in FRAGMENTS_SHOT:
            ...
            if not os.path.isfile(fragment_filename):
                ...
                pipe_globals.stage_end(fragment_stage)
            shot_stage.GetRootLayer().subLayerPaths.insert(
                0, pipe_globals.anchor_path(fragment_filename, shot_stage.GetRootLayer().identifier)
            )
