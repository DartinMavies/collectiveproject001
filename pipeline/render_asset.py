...
def main(args):
    ...
    for asset in assets:
        ...
        stage = pipe_globals.stage_begin(
            render_usd_filename, start=0.0, end=float(frameend - framestart + 1)
        )
        stage.GetRootLayer().subLayerPaths = [
            pipe_globals.anchor_path(asset_filename, stage.GetRootLayer().identifier)
        ]
        (camera_xform_prim, camera_prim) = pipe_globals.create_default_camerarig(
        ...
        rendervar_prim = pipe_globals.create_rendervar(
            stage, aov_name="rgba", aov_source="rgba", aov_format=""
        )
        ...
        renderproduct_prim = pipe_globals.create_renderproduct(
            ...
            product_filepath="placeholder.jpg",
        ...
        reviewer_stage = pipe_globals.stage_begin(
            reviewer_filename, start=0.0, end=float(frameend - framestart + 1)
        )
        pipe_globals.make_reviewer_per_purpose(
            reviewer_stage,
            purpose="proxy",
            textures="output_proxy.####.jpg",
