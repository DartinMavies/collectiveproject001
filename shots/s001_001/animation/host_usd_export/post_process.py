...
module_path = os.path.join(
    os.path.dirname(__file__), "..", "..", "..", "..", "pipeline", "pipe_globals.py"
)
...
#
animation_layer_path = os.path.join(
    os.path.dirname(__file__), "..", "..", "odie01", "animation", "animated_proxy.usd"
)
...
animation_layer_frame_end = (
    animation_layer_frame_start + (anim_frame_end - anim_frame_start) + 100.0
)
animation_layer_stage = pipe_globals.stage_begin(
    animation_layer_path,
    start=animation_layer_frame_start,
    end=animation_layer_frame_end,
    fps=anim_fps,
)
...
animation_frag_path = os.path.join(
    os.path.dirname(__file__), "..", "..", "odie01", "animation", "index.usda"
)
frag_stage = pipe_globals.stage_begin(
    animation_frag_path,
    start=animation_layer_frame_start,
    end=animation_layer_frame_end,
    fps=anim_fps,
...
camera_animation_layer_path = os.path.join(
    os.path.dirname(__file__), "..", "..", "renderCam", "animation", "index.usda"
)
camera_animation_stage = pipe_globals.stage_begin(
    camera_animation_layer_path,
    start=animation_layer_frame_start,
    end=animation_layer_frame_end,
    fps=anim_fps,
)
...
camera_prim = UsdGeom.Camera.Define(
    camera_animation_stage, main_xform.GetPath().AppendChild("mono")
)
